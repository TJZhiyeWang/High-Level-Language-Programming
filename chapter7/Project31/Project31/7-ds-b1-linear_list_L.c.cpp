...
/* 删除线性表 */
Status DestroyList(LinkList *L, Status(*MySubFree)(LinkList *L), Status(*MyFree)(ElemType p))
{
	LinkList q, p = *L; //指向首元
...
Status NextElem(LinkList L, ElemType cur_e, ElemType *next_e, Status(*compare)(ElemType e1, ElemType e2), Status(*assign)(ElemType *dst, ElemType src))
{
	...
	(*assign)(next_e, p->next->data);
	return OK;
}

/* 在指定位置前插入一个新元素 */
Status ListInsert(LinkList *L, int i, ElemType e, Status(*assign)(ElemType *dst, ElemType src), Status(*MyAlloc)(ElemType *p, ElemType e))
{
...
Status ListDelete(LinkList *L, int i, ElemType *e, Status(*MySubFree)(LinkList *L), Status(*MyFree)(ElemType p), Status(*assign)(ElemType *dst, ElemType src))
{
	...
	(*assign)(e, q->data); //取第i个结点的值
	(*MySubFree)(&q);

	return OK;
}
/* 遍历线性表 */
Status ListTraverse(LinkList L, Status(*MyVisit)(ElemType e))
{
	extern int line_count; //在main中定义的打印换行计数器（与算法无关）
	LinkList p = L->next;

	line_count = 0;		//计数器恢复初始值（与算法无关）
	while (p && (*MyVisit)(p->data) == TRUE)
		p = p->next;
